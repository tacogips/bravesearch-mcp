...
// Example client for CrateDocs-MCP
//
// This example demonstrates how to interact with the CrateDocs MCP server using both:
...
async fn http_sse_client() -> Result<()> {
    ...
    let _server_handle = tokio::spawn(async {
        match tokio::process::Command::new("cargo")
            .args(["run", "--bin", "bravesearch-mcp", "http", "--address", "127.0.0.1:8081"])
            .spawn() 
        {
            Ok(mut child) => {
    ...
    let server_check = client.get("http://127.0.0.1:8081/health")
    ...
    match server_check {
        ...
        Err(_) => {
            ...
            println!("Note: This example expects a health endpoint at /health, which might not exist.");
    ...
    println!("Sending initialized notification...");
    let notify_response = client.post(&init_url).json(&init_notify_request).send().await?;
    ...
    println!("Sending request to list available tools...");
    let tools_response = client.post(&init_url).json(&list_tools_request).send().await?;
